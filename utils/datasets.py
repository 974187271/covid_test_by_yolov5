...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):
            ...
            self.imgs[i] = temp  # guarantee first frame
            # self.imgs[i] = cv2.resize(self.imgs[i], (416, 416))
        ...
        s = np.stack([letterbox(x, self.img_size, stride=self.stride).shape for x in self.imgs], 0)  # shapes
    ...
    def __next__(self):
        ...
        # Letterbox
        img = [letterbox(x, self.img_size, auto=self.rect, stride=self.stride) for x in img0]
